...
- (void)cashRequest{
    ...
    [SearchViewModel getFeeSuccess:^(NSString *msg, Cash *cash) {
        if (self.showType == ShowType_Person) {
            _moneyLabel.text = [cash.rcRmb stringByAppendingString:@"元"];
        }else if (self.showType == ShowType_Project){
            _moneyLabel.text = [cash.proRmb stringByAppendingString:@"元"];
        }else if (self.showType == ShowType_Message){
            _moneyLabel.text = [cash.rcRmb stringByAppendingString:@"元"];
        }
...
- (void)layoutSubviews{
    ...
    if (self.showType != ShowType_Message) {
        ...
        [phoneBtn addTarget:self action:@selector(phoneClick) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:phoneBtn];
        phoneBtn.maker.rightTo(self.view, 16).centerYTo(moneyIcon, 0).widthEqualTo(80).heightEqualTo(40);
    }
