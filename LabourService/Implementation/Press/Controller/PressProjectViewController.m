...
- (void)addKind {
    ...
    selectVC.selectKindMaxPersonNumLimited = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    KindWithNumCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
...
- (UITableView *)kindsTableView{
    if (!_kindsTableView) {
        _kindsTableView  = [UITableView new];
        _kindsTableView.delegate = self ;
        _kindsTableView.dataSource = self ;
        _kindsTableView.rowHeight = 30 ;
        _kindsTableView.tableFooterView = [UIView new];
        [_kindsTableView registerClass:[KindWithNumCell class] forCellReuseIdentifier:@"cell"];
    }
    return _kindsTableView ;
}
...
//    _contentV.backgroundColor = [UIColor redColor];
    ...
    [_kindsView addSubview:self.kindsTableView];
    _kindsTableView.maker.topTo(kindT, 0).leftTo(_kindsView, 0).rightTo(_kindsView, 0).heightEqualTo(1);

    UILabel * introduceT = [[UILabel alloc]init];
    introduceT.text = @"项目简介";
